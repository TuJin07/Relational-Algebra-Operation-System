...
> 在这里写一两个你自己用到的数据结构，比如说计算结果表这个数据结构是怎么存储的，代码贴不贴都可以
...
@薛晴 还是写一下页面中用到的数据结构，包括名称、用途

#### 源程序文件说明

@薛晴 该功能模块的源程序文件以及功能说明，把页面对应的代码分段贴一下即可，比如说“这一块是关系管理部分，用户可以在此对关系进行增删；这一块是表达式输入部分…”

#### 主要函数说明

@薛晴 点击按钮等主要函数都可以说，包括函数名称、功能、格式、参数等说明
...
@薛晴 贴一下前端交互接口的代码，可以删掉一点详细实现，不需要一字不差贴上来，简洁点
